...
protected:
	...
	virtual void operate() {
		...
		jp[i1] = amp * std::cos(theta) + jp_0[i1];
		jp[i2] = amp * (std::sin(2*theta)/2 - 1.0) + jp_0[i2];
...
protected:
	...
	virtual void operate() {
		...
		cp[1] = amp * std::sin(2*theta)/2 + y_0;
...
int wam_main(int argc, char** argv, ProductManager& pm, systems::Wam<DOF>& wam) {
  ...
  // Set start position, depending on robot type and configuration.
	...
	printf("Press [Enter] to move the end-point in figure 8 using joint position control.");
	waitForEnter();
	...
	printf("Press [Enter] to move the end-point in figure 8 using Cartesian position control.");
	waitForEnter();
